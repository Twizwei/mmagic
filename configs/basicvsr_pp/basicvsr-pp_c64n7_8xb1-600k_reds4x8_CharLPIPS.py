...
experiment_name = 'basicvsr-pp_c64n7_8xb1-600k_reds4x8_CharLPIPS'
...
train_pipeline = [
    ...
    dict(type='SetValues', dictionary=dict(scale=8)),
...
val_pipeline = [
    ...
    dict(type='LoadImageFromFile', key='gt', channel_order='rgb'),
    dict(type='SetValues', dictionary=dict(scale=8)),
    dict(type='PairedRandomCrop', gt_patch_size=(1280, 720)),
    dict(type='PackInputs')
]
...
model = dict(
    ...
    # pixel_loss=dict(type='CharLPIPSLoss', loss_weight=1.0,  char_weight=1.0, lpips_weight=0.5,),
    # perceptual_loss=dict(type='LPIPSLoss', loss_weight=0.5, net='vgg'),
    perceptual_loss=0.0,
